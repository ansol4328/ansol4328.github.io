...
> 첫번째 식은 다음과 같다. $x\equiv{1}\pmod{2}$, $x\equiv{1}\pmod{4}$, $x\equiv{2}\pmod{3}$   
> 두번째 식은 다음과 같다. $x\equiv{1}\pmod{2}$, $x\equiv{2}\pmod{3}$, $x\equiv{2}\pmod{9}$   
> 두 식을 비교하면 서로 모순인 관계(=같은 법에 대하여 서로 다른 나머지를 갖는 경우) 가 없으므로, 아래 식들을 푸는 문제와 동치이다.   
>
> $$x\equiv{1}\pmod{2}$$
>
> $$x\equiv{2}\pmod{3}$$
...
> $$x\equiv{2}\pmod{9}$$
>
> 또한 위의 식은 pairwise coprime 관계를 유지하는 식들만 남겨도 된다. 무슨 말이냐면 각각의 식은 소수의 $k$ 제곱 꼴이므로, 각 소수별로 가장 차수가 높은 식에 대하여만 풀어도 된다. 따라서 아래 식을 풀도록 하자.
>
> $$x\equiv{1}\pmod{4}$$
>
> $$x\equiv{2}\pmod{9}$$
...
풀이 과정에서 알 수 있지만, **합동식 간에 모순 관계가 존재한다면 자명히 해는 존재하지 않는다.** 모순 관계가 존재하지 않으면, 가장 높은 차수의 법으로만 이루어진 pairwise coprime인 연립 합동식을 CRT를 이용하여 풀면 된다.
